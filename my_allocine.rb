...
requests["Find the movie which was released in the year 1999"] = "SELECT mov_title FROM movies WHERE mov_dt_rel BETWEEN '1999-01-01' AND '1999-12-31';"
...
requests["Find the actors with all information who played a role in the movies 'Annie Hall'"] = "SELECT act_id, act_fname, act_lname, act_gender FROM actors JOIN movies_actors ON actors.id = movies_actors.act_id JOIN movies ON movies_actors.mov_id = movies.id WHERE mov_title = 'Annie Hall';"
...
requests["Find the name of movie and director who directed a movies that casted a role as Sean Maguire"] = "SELECT dir_fname, dir_lname, mov_title FROM movies JOIN movies_actors ON movies.id = movies_actors.mov_id JOIN directors_movies ON movies_actors.mov_id = directors_movies.mov_id JOIN directors ON directors_movies.dir_id = directors.id WHERE role = 'Sean Maguire';"
